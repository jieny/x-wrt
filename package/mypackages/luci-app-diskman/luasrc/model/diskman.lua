...
local df = luci.sys.exec("/bin/df 2>/dev/null") or ""
...
local get_smart_info = function(device)
  local section
  local smart_info = {}
  for _, line in ipairs(luci.util.execl("/usr/sbin/smartctl -H -A -i -n standby -f brief /dev/" .. device)) do
    local attrib, val
    if section == 1 then
...
local get_parted_info = function(device)
  ...
  for line in luci.util.execi("/sbin/parted -s -m /dev/" .. device .. " unit s print free", "r") do
    ...
    elseif line:find("^%d-:.+") then
      ...
      if partition_temp["number"] > 0 and partition_temp["fs"] == "" then
        partition_temp["fs"] = luci.util.exec("/usr/bin/lsblk /dev/"..device.. tostring(partition_temp["number"]) .. " -no fstype"):match("([^%s]+)") or ""
      end
...
d.create_raid = function(rname, rlevel, rmembers)
  ...
  local cmd = "/sbin/mdadm --create "..rname.." --run --assume-clean --homehost=any --level=" .. rlevel .. " --raid-devices=" .. #rmembers .. " " .. table.concat(rmembers, " ")
...
d.list_btrfs_devices = function()
  local btrfs_device = {}
  local line, _uuid
  for _, line in ipairs(luci.util.execl("/usr/bin/btrfs filesystem show -d --raw"))
  do
...
d.create_btrfs = function(blabel, blevel, bmembers)
  ...
  if type(bmembers) ~= "table" or next(bmembers) == nil then return "ERR no btrfs support or no members" end
...
d.get_btrfs_info = function(m_point)
  local btrfs_info = {}
  if not m_point then return btrfs_info end
  local cmd = "/usr/bin/btrfs filesystem show --raw " .. m_point
  ...
  cmd = "/usr/bin/btrfs filesystem usage -b " .. m_point
...
local subvolume = {}
if not m_point then return subvolume end

-- get default subvolume
local cmd = "/usr/bin/btrfs subvolume get-default " .. m_point
local res = luci.util.exec(cmd)
...
if not snapshot then
  ...
  cmd = "/usr/bin/btrfs subvolume show ".. m_point
  for _, line in ipairs(luci.util.execl(cmd)) do
...
cmd = "/usr/bin/btrfs subvolume list -gcu" .. (snapshot and "s " or " ") .. m_point
for _, line in ipairs(luci.util.execl(cmd)) do
  ...
  if id and gen and top_level and uuid and path then
    ...
    if not snapshot then
      ...
      local show_cmd = "/usr/bin/btrfs subvolume show "..m_point.."/"..path
